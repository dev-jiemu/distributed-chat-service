...
Pod가 다운되면 `localSessions`는 통째로 사라지지만, Redis에는 **"이 유저는 다운된 서버에 연결되어 있다"**는 정보가 남아요.
다른 서버가 이 유저에게 메세지를 라우팅하려고 Redis를 조회하면 다운된 서버를 향해 전달을 시도하고, 그 결과 **메세지가 유실**됩니다.
또한 Pod가 갑작스럽게 종료되면 `afterConnectionClosed()`가 실행되지 않아 **Redis에 죽은 세션 정보가 남아요** (TTL 30분까지).

가정
- Server 역할을 하는 Pod 가 총 3개 떠있고, 그중 server1 의 pod 가 내려갔을 경우
  - localSession 은 사라짐
  - redis 는 정보가 남아있지만, 다운된 서버로 연결되어있음으로 남음(...)
